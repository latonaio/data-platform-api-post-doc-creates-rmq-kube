...
import (
	"context"
	"crypto/sha256"
	dpfm_api_input_reader "data-platform-api-post-doc-creates-rmq-kube/DPFM_API_Input_Formatter"
	dpfm_api_output_formatter "data-platform-api-post-doc-creates-rmq-kube/DPFM_API_Output_Formatter"
	"data-platform-api-post-doc-creates-rmq-kube/config"
	"encoding/base64"
	"encoding/hex"
	"fmt"
...
func (c *DPFMAPICaller) AsyncDocCreates(
	...
	var fileExtension = input.Post.HeaderDoc.FileExtension
	var docData = input.DocData

	var combinedString = fmt.Sprintf(
		"%v%v",
		time.Now().Unix(),
		input.Post.HeaderDoc.DocVersionID,
	)
	...
	response := c.createSqlProcess(input, output, &dpfm_api_output_formatter.HeaderDoc{
		...
		DocVersionID:             input.Post.HeaderDoc.DocVersionID,
		DocID:                    &dockId,
		FileExtension:            input.Post.HeaderDoc.FileExtension,
		FileName:                 input.Post.HeaderDoc.FileName,
		FilePath:                 directoryPath,
		DocIssuerBusinessPartner: input.Post.HeaderDoc.DocIssuerBusinessPartner,
	}, errs, log)
